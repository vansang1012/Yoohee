$(window).load(function(){
	...
	// Xóa các module đơn lẻ
	$('.cms-delete_picsaca-ajax').click(function(){
		_this = $(this);
		if($('.check-all:checked').length == 0){
			alert('Bạn chưa chọn đối tượng.');
			return false;
		...
		if(confirm('Đối tượng bạn lựa chọn sẽ bị xóa?\n\n==[HÃY THẬT CẨN THẬN VỚI THAO TÁC NÀY!]==')){
			$('#cms-mask').show();
			$.post(cms_url+cms_backend+'/common/delete_picsaca/'+_this.attr('name')+'.html', $('#frmView').serialize(), function(data){
				$('#cms-mask').hide();
				location.reload();
			});
			return false;
	...
	// Xóa các craw đơn lẻ
	$('.cms-delete_craw-ajax').click(function(){
		_this = $(this);
		if($('.check-all:checked').length == 0){
			alert('Bạn chưa chọn đối tượng.');
			return false;
		...
		if(confirm('Đối tượng bạn lựa chọn sẽ bị xóa?\n\n==[HÃY THẬT CẨN THẬN VỚI THAO TÁC NÀY!]==')){
			$('#cms-mask').show();
			$.post(cms_url+cms_backend+'/common/delete_craw/'+_this.attr('name')+'.html', $('#frmView').serialize(), function(data){
				$('#cms-mask').hide();
				location.reload();
			});
			return false;
	...
	// Cào dữ liệu
	$('.cms-caodulieu-ajax').click(function(){
		_this = $(this);
		if($('.check-all:checked').length == 0){
			alert('Bạn chưa chọn đối tượng.');
			return false;
		}
		if(confirm('Dữ liệu sẽ được lấy về bởi đối tượng đã chọn ?\n\n==[XEM LẠI NHỮNG MỤC ĐÃ CHỌN!]==')){
			$('#cms-mask').show();
			$.post(cms_url+cms_backend+'/common/caodulieu/'+_this.attr('name')+'.html', $('#frmView').serialize(), function(data){
				$('#cms-mask').hide();
				location.reload();
			});
			return false;
	...
	$('.cms-keke-ajax').click(function(){
		...
		if(confirm('Dữ liệu sẽ được lấy về bởi đối tượng đã chọn ?\n\n==[XEM LẠI NHỮNG MỤC ĐÃ CHỌN!]==')){
			$('#cms-mask').show();
			$.post(cms_url+cms_backend+'/common/craw_picsaca/'+_this.attr('name')+'.html', $('#frmView').serialize(), function(data){
				$('#cms-mask').hide();
				location.reload();
			});
			return false;
	...
	// Ghi dữ liệu vào đâu đó
	$('.cms-ghidulieu-ajax').click(function(){
		_this = $(this);
		if($('.check-all:checked').length == 0){
			alert('Bạn chưa chọn đối tượng.');
			return false;
		}
		if(confirm('Đối tượng bạn lựa chọn sẽ được ghi vào ...?\n\n==[HÃY CHÚ Ý THAO TÁC NÀY!]==')){
			$('#cms-mask').show();
			$.post(cms_url+cms_backend+'/common/ghidulieu/'+_this.attr('name')+'.html', $('#frmView').serialize(), function(data){
				$('#cms-mask').hide();
				location.reload();
			});
			return false;
	...
	// AJAX UPLOAD
	$(function(){
	var btnUpload=$('#btnimg');//Nut Upload
	var status=$('#lbfile');//Trạng thai
	new AjaxUpload ( btnUpload, {
		action: cms_url+'admin/picsaca/do_upload/',
		name: 'nfile',
		data: { dir:'./upload/'},
